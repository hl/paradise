defmodule ParadiseWeb.AstronautResetPasswordLiveTest do
  ...
  setup do
    ...
    %{token: token, astronaut: astronaut}
  end

  describe "Reset password page" do
    test "renders reset password with valid token", %{conn: conn, token: token} do
      {:ok, _lv, html} = live(conn, ~p"/astronauts/reset_password/#{token}")

      assert html =~ "Reset Password"
    end

    test "does not render reset password with invalid token", %{conn: conn} do
      {:error, {:redirect, to}} = live(conn, ~p"/astronauts/reset_password/invalid")
    ...
    test "renders errors for invalid data", %{conn: conn, token: token} do
      {:ok, lv, _html} = live(conn, ~p"/astronauts/reset_password/#{token}")

      result =
        lv
        |> element("#reset_password_form")
        |> render_change(
          astronaut: %{"password" => "secret12", "password_confirmation" => "secret123456"}
        )
  ...
  describe "Reset Password" do
    test "resets password once", %{conn: conn, token: token, astronaut: astronaut} do
      {:ok, lv, _html} = live(conn, ~p"/astronauts/reset_password/#{token}")
